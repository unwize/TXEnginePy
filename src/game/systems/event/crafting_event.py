...
class CraftingEvent(Event):
    ...
    def __init__(self):
        ...
        def _logic(user_input: int):
            ...
            if self._player_ref.crafting_controller.has_sufficient_ingredients(
                self._chosen_recipe
            ) and recipe_manager.get_recipe(self._chosen_recipe).is_requirements_fulfilled(self._player_ref):
        ...
        @self.state_content(self.States.DISPLAY_RECIPES)
        def _content():
        ...
        @self.state_content(self.States.NUM_CRAFTS)
        def _content() -> dict:
            return ComponentFactory.get(
                [
        ...
        @self.state_logic(self.States.CONFIRM_RECIPE, InputType.AFFIRMATIVE)
        def _logic(user_input: bool):
            self.set_state(self.States.EXECUTE_RECIPE if user_input else self.States.DEFAULT)

        @self.state_content(self.States.CONFIRM_RECIPE)
        def _content():
            return ComponentFactory.get(
                [
        ...
        @self.state_logic(self.States.INSUFFICIENT_INGREDIENTS, InputType.ANY)
        def _logic(_: any):
        ...
        @self.state_content(self.States.INSUFFICIENT_INGREDIENTS)
        def _content():
            return ComponentFactory.get(
